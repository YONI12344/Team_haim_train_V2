...
create table athlete_profiles (
  ...
  race_pace_3000 text,
  race_pace_5000 text,
  race_pace_10000 text,
  race_pace_half_marathon text,
  race_pace_marathon text,
  pr_1500 text,
  pr_2000 text,
  pr_3000 text,
  pr_5000 text,
  pr_10000 text,
  pr_half_marathon text,
  pr_marathon text,
  goal_1500 text,
  goal_2000 text,
  goal_3000 text,
...
create policy "Athletes view own extended profile" on athlete_profiles
  for select using (athlete_id = auth.uid() or get_user_role() = 'coach');

create policy "Athletes update own extended profile" on athlete_profiles
  for update using (athlete_id = auth.uid() or get_user_role() = 'coach');

create policy "Athletes insert own extended profile" on athlete_profiles
  for insert with check (athlete_id = auth.uid() or get_user_role() = 'coach');

-- messages policies
create policy "Users view their chat messages" on messages
  for select using (athlete_id = auth.uid() or sender_id = auth.uid() or get_user_role() = 'coach');

create policy "Users send chat messages" on messages
  for insert with check (athlete_id = auth.uid() or sender_id = auth.uid() or get_user_role() = 'coach');
